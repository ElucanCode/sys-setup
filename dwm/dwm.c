...
static void setlightscheme(const Arg *arg);
static void setdarkscheme(const Arg *arg);
static void switchlightdark(const Arg *arg);
...
void
setlightscheme(const Arg *arg)
{
    (void)arg; /* ignores the argument */
    Arg off;
    if (!darkscheme)
        return;
    off = (Arg){ .i = darkfirst ? +1 : -1 };
    darkscheme = 0;
    setscheme(&off);
}

void setdarkscheme(const Arg *arg)
{
    (void)arg; /* ignores the argument */
    Arg off;
    if (darkscheme)
...
void
switchlightdark(const Arg *arg)
{
    (void)arg; /* ignores the argument */
    Arg off;
    if ((darkscheme && darkfirst) || (!darkscheme && !darkfirst))
        off = (Arg){ .i = +1 };
    else if ((darkscheme && !darkfirst) || (!darkscheme && darkfirst))
        off = (Arg){ .i = -1 };
